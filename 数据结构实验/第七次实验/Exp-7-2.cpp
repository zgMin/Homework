...
bool bfs(ALGraph G,int v,void (*visit)(char));			//广度优先遍历 
...
bool bfs(ALGraph G,int v,void (*visit)(char)){
	...
	while(front!=end){//队不空
		 ...
		 while(p){
			if(!visited[p->adjvex]){		//如果该点未被访问，访问该点并入队 
				...
				visited[p->adjvex]=visited[tmp]%2+1;//打上不同于邻接点的标记 
				q[end++]=p->adjvex;//入队
			} 
			else{//如果被访问过，判断颜色标记是否一样，一样则返回false 
				if(visited[tmp]==visited[p->adjvex]) 
				return false;
			}
			p=p->nextarc;			//下一个邻接点 
		}
